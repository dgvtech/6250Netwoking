    ...
    def __init__(self, n=1, cpu=.1, bw=10, delay=None,
        ...
        if n > 1:
            for n in range(n-1): 
                ...
                Host= self.addHost(hName, **hconfig) 
                self.addLink(switchLast, Switch, 
                             port1=downlink, port2=uplink, **lconfig) 
                self.addLink(Host, Switch, 
                             port1=0, port2=hostlink, **lconfig) 
                switchLast = switchName 
...
def run_parkinglot_expt(net, n):
    ...
    for n in range(n): 
         n += 1 
         nNode = 'h' + str(n) 
         theNode = net.getNodeByName(nNode) 
         theNode.sendCmd('iperf -c %s -p %s -t %d -i 1 -yc > %s/iperf_%s.txt' % (recvr.IP(), 5001, seconds, args.dir, nNode))         
